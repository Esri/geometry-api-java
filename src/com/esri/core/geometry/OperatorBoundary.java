...
public abstract class OperatorBoundary extends Operator {
	 ...
	 *\return Returns a cursor over boundries for each geometry.
	 */
	 ...
	 *For Polyline - returns a multipoint, using OGC specification (includes path endpoints, usinng mod 2 rule).
	...
	public static OperatorBoundary local() {
		return (OperatorBoundary) OperatorFactoryLocal.getInstance()
				.getOperator(Type.Boundary);
	}
}
