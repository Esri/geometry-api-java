...
class OperatorImportFromGeoJsonLocal extends OperatorImportFromGeoJson {
  ...
  @Override
  public MapGeometry execute(
    int importFlags,
    Geometry.Type type,
    String geoJsonString,
    ProgressTracker progressTracker
  ) throws JsonGeometryException {
    MapGeometry map_geometry = OperatorImportFromGeoJsonHelper.importFromGeoJson(
      importFlags,
      type,
      JsonParserReader.createFromString(geoJsonString),
      progressTracker,
      false
    );
    return map_geometry;
  ...
  public MapGeometry execute(
    ...
    return OperatorImportFromGeoJsonHelper.importFromGeoJson(
      importFlags,
      type,
      jsonReader,
      progressTracker,
  ...
  static final class OperatorImportFromGeoJsonHelper {
    ...
    static MapGeometry importFromGeoJson(
      int importFlags,
      Geometry.Type type,
      ...
      if (
        geo_json_helper.m_ogcType == GeoJsonValues.GeometryCollection &&
        !skip_coordinates
      ) throw new JsonGeometryException("parsing error");

      return new MapGeometry(
        ms.m_ogcStructure.m_geometry,
        ms.m_spatialReference
      );
    }

    static MapOGCStructure importFromGeoJson(
      int importFlags,
      Geometry.Type type,
      JsonReader json_iterator,
      ProgressTracker progress_tracker,
    ...
    private void import_coordinates_(
      JsonReader json_iterator,
      ProgressTracker progress_tracker
    ) throws JsonGeometryException {
      ...
      while (json_iterator.currentToken() != JsonReader.Token.END_ARRAY) {
        if (isDouble_(json_iterator)) {
          ...
          json_iterator.currentToken() == JsonReader.Token.START_ARRAY
        ) {
        ...
        if (
          coordinates_level_lower == coordinates_level_upper &&
          coordinates_level_lower == 1
        ) { // special
          // code
          // for
          // Points
          ...
          while (json_iterator.currentToken() != JsonReader.Token.END_ARRAY) {
            ...
            if (
              ...
              while (
                json_iterator.currentToken() != JsonReader.Token.END_ARRAY
              ) {
                if (isDouble_(json_iterator)) {
    ...
    private void addCoordinate_(JsonReader json_iterator)
      ...
      if (m_position == null) {
        m_position =
          (AttributeStreamOfDbl) AttributeStreamBase.createDoubleStream(0);
      }
      ...
      if (isDouble_(json_iterator)) {
        if (!m_b_has_zs_known) {
          ...
          if (!m_b_has_zs) {
            m_zs =
              (AttributeStreamOfDbl) AttributeStreamBase.createDoubleStream(
                size >> 1,
    ...
    private void addPath_() {
      if (m_paths == null) {
        m_paths =
          (AttributeStreamOfInt32) AttributeStreamBase.createIndexStream(0);
      }
    ...
    private void addPathFlag_(boolean b_polygon_start) {
      if (m_path_flags == null) {
        m_path_flags =
          (AttributeStreamOfInt8) AttributeStreamBase.createByteStream(0);
      }
    ...
    private double readDouble_(JsonReader json_iterator)
      ...
      if (
        current_token == JsonReader.Token.VALUE_NULL ||
        (
          current_token == JsonReader.Token.VALUE_STRING &&
    ...
    private Geometry createGeometry_(GeoJsonType geo_json_type, int type)
      ...
      if (type != Geometry.GeometryType.Unknown) {
        switch (type) {
          case Geometry.GeometryType.Polygon:
            if (
              geo_json_type != GeoJsonType.MultiPolygon &&
          ...
          case Geometry.GeometryType.Polyline:
            if (
              geo_json_type != GeoJsonType.MultiLineString &&
      ...
      if (
        geo_json_type == GeoJsonType.GeometryCollection
      ) throw new IllegalArgumentException("invalid argument");
    ...
    private Geometry createPolygonFromStreams_() {
      ...
      assert (m_paths != null);
      assert (
        (m_num_embeddings == 3 && m_path_flags == null) ||
        (m_num_embeddings == 4 && m_path_flags != null)
      );
      ...
      if (m_path_flags == null) {
        m_path_flags =
          (AttributeStreamOfInt8) AttributeStreamBase.createByteStream(
            m_paths.size(),
            (byte) 0
          );
        m_path_flags.setBits(
          0,
          (byte) (PathFlags.enumClosed | PathFlags.enumOGCStartPolygon)
        );
      ...
      multi_path_impl.notifyModified(
        MultiVertexGeometryImpl.DirtyFlags.DirtyAll
      );
    ...
    private Geometry createPolylineFromStreams_() {
      assert (m_position != null);
      assert (
        (m_num_embeddings == 2 && m_paths == null) ||
        (m_num_embeddings == 3 && m_paths != null)
      );
    ...
    private Geometry createMultiPointFromStreams_() {
      ...
      multi_point_impl.notifyModified(
        MultiVertexGeometryImpl.DirtyFlags.DirtyAll
      );
      return multi_point;
    }
    ...
    private void checkPathPointCountsForMultiPath_(boolean b_is_polygon) {
      ...
      AttributeStreamOfDbl adjusted_position = (AttributeStreamOfDbl) AttributeStreamBase.createDoubleStream(
        m_position.size() - guess_adjustment
      );
      AttributeStreamOfInt32 adjusted_paths = (AttributeStreamOfInt32) AttributeStreamBase.createIndexStream(
        m_paths.size()
      );
    ...
    private void insertIntoAdjustedStreams_(
      ...
      AttributeStreamOfDbl adjusted_ms,
      int adjusted_start,
      int path_start,
      int count
    ) {
      adjusted_position.insertRange(
        adjusted_start * 2,
        m_position,
        path_start * 2,
        count * 2,
      ...
      if (spatial_reference == null) {
        try {
          if (b_found_esriwkt) { // I exported crs wkt strings like
            ...
            // name is like
            // "ESRI:<wkt>"
            String potential_wkt = GeoJsonCrsTables.getWktFromCrsName(
              crs_identifier_name
            );
  ...
  @Override
  public MapOGCStructure executeOGC(
    int import_flags,
    String geoJsonString,
    ProgressTracker progress_tracker
  ) throws JsonGeometryException {
