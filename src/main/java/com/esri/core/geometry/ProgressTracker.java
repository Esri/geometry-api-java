...
public abstract class ProgressTracker {
     ...
     *         is the number of steps the operation is expects to complete its task.
     *
     * @return {@code true} if the operation can continue. Returns {@code false}
     *         when the operation has to terminate due to a user cancelation.
     */
    public abstract boolean progress(int step, int totalExpectedSteps);

    /**
     * Checks the tracker and throws UserCancelException if tracker is not null
     * and progress returns false
     *
     * @param  tracker
